...
export default function CheckInPage() {
  ...
  const slider = (label: string, key: "progressScore"|"energyScore"|"moodScore") => (
    <div key={key}>
      ...
      <input type="range" min={1} max={10} value={data[key]} onChange={set(key)} style={{marginTop:"0.5rem"}} />
    </div>
  )
  ...
  const steps = [
    ...
    <div key={0} className="flex flex-col gap-5">
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:"1rem"}}>
        {[["Current Weight (lbs)","weight"],["Body Fat % (opt)","bodyFat"],["Muscle % (opt)","musclePct"]].map(([l,k]) => (
          <div key={k}><label>{l}</label><input type="number" placeholder="—" value={data[k as keyof typeof data] as string} onChange={set(k as keyof typeof data)} /></div>
        ))}
      </div>
    ...
    <div key={1} className="flex flex-col gap-4">
      ...
      <div className="flex flex-wrap gap-2">
        {SIDE_EFFECTS.map(s => (
          <button key={s} type="button" onClick={() => toggleSE(s)}
            style={{
    ...
    <div key={2} className="flex flex-col gap-4">
      ...
      <label style={{display:"flex",gap:"0.75rem",alignItems:"flex-start",cursor:"pointer",marginTop:"1rem"}}>
        ...
        <span style={{fontSize:"0.9rem",lineHeight:1.6}}>
          <span style={{color:"red",fontWeight:700"}}>⚠ Request urgent follow-up</span> — I have a concern that needs prompt attention.
        </span>
      </label>
    </div>,
    /* 3 Notes */
