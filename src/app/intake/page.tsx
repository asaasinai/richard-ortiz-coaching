...
export default function IntakePage() {
  ...
  // Generic setter — preserves cursor position for text inputs
  const setField = useCallback(<K extends keyof typeof initialData>(k: K, v: (typeof initialData)[K]) => {
    setData(p => ({ ...p, [k]: v }))
  }, [])

  const handleChange = (k: keyof typeof data) => (e: React.ChangeEvent<HTMLInputElement|HTMLTextAreaElement|HTMLSelectElement>) => {
  ...
  const submit = async () => {
    ...
    const payload = {
      ...data,
      height: data.heightFt ? `${data.heightFt}'${data.heightIn || 0}"` : ""
    }
  ...
  // Reusable labeled text input — stable cursor via uncontrolled-friendly pattern
  const Field = ({ label, id, type="text", placeholder="", value, onChange, autoComplete="off" }: {
  ...
  const TextArea = ({ label, id, rows=3, placeholder="", value, onChange }: {
    label: string, id: string, rows?: number, placeholder?: string,
    value: string, onChange: (e: React.ChangeEvent<HTMLTextAreaElement>) => void
  }) => (
    <div>
      <label htmlFor={id}>{label}</label>
      <textarea id={id} name={id} rows={rows} placeholder={placeholder} value={value} onChange={onChange} />
    </div>
  )

  // Sleep stepper: integer 1–20, +/- buttons
  const SleepStepper = () => (
    <div>
      <label>Average Sleep (hours/night)</label>
      <div style={{ display:"flex",alignItems:"center",gap:"0",marginTop:"0.4rem",width:"fit-content",border:"1px solid var(--border)",borderRadius:"var(--radius)",overflow:"hidden" }}>
        <button type="button"
          onClick={() => setField("sleepHrs", Math.max(1, data.sleepHrs - 1))}
          style={{ width:"2.5rem",height:"2.5rem",background:"var(--surface-2)",border:"none",color:"var(--text)",cursor:"pointer",fontSize:"1.1rem",display:"flex",alignItems:"center",justifyContent:"center" }}>
          <Minus size={14}/>
        </button>
        <div style={{ width:"3rem",textAlign:"center",fontWeight:700,fontSize:"1rem",color:"var(--text)",lineHeight:"2.5rem",background:"var(--surface)",userSelect:"none" }}>
          {data.sleepHrs}
        </div>
        <button type="button"
          onClick={() => setField("sleepHrs", Math.min(20, data.sleepHrs + 1))}
          style={{ width:"2.5rem",height:"2.5rem",background:"var(--surface-2)",border:"none",color:"var(--text)",cursor:"pointer",fontSize:"1.1rem",display:"flex",alignItems:"center",justifyContent:"center" }}>
          <Plus size={14}/>
        </button>
      </div>
      <p style={{ fontSize:"0.75rem",color:"var(--text-mute)",marginTop:"0.4rem" }}>Range: 1–20 hours</p>
    </div>
  )
  ...
  const steps = [
    ...
    <div key={0} className="flex flex-col gap-4">
      <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:"1rem" }}>
        <Field label="First Name" id="firstName" placeholder="Richard" value={data.firstName} onChange={handleChange("firstName")} autoComplete="given-name" />
        <Field label="Last Name" id="lastName" placeholder="Ortiz" value={data.lastName} onChange={handleChange("lastName")} autoComplete="family-name" />
      </div>
      <Field label="Email" id="email" type="email" placeholder="you@example.com" value={data.email} onChange={handleChange("email")} autoComplete="email" />
      <Field label="Phone" id="phone" type="tel" placeholder="+1 (555) 000-0000" value={data.phone} onChange={handleChange("phone")} autoComplete="tel" />
      <Field label="Date of Birth" id="dob" type="date" value={data.dob} onChange={handleChange("dob")} autoComplete="bday" />
    </div>,
    ...
    <div key={1} className="flex flex-col gap-5">
        ...
        <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:"0.75rem",marginTop:"0.4rem" }}>
          <div style={{ position:"relative" }}>
            <input id="heightFt" name="heightFt" type="number" min={0} max={8} placeholder="5"
              value={data.heightFt} onChange={handleChange("heightFt")}
          ...
          <div style={{ position:"relative" }}>
            <input id="heightIn" name="heightIn" type="number" min={0} max={11} placeholder="10"
              value={data.heightIn} onChange={handleChange("heightIn")}
              style={{ paddingRight:"2.25rem" }} />
      ...
      <Field label="Weight (lbs)" id="weight" placeholder="180" value={data.weight} onChange={handleChange("weight")} />
      <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:"1rem" }}>
        <Field label="Body Fat % (optional)" id="bodyFat" placeholder="20" value={data.bodyFat} onChange={handleChange("bodyFat")} />
        <Field label="Muscle Mass % (optional)" id="musclePct" placeholder="40" value={data.musclePct} onChange={handleChange("musclePct")} />
      </div>
    </div>,

    /* 2 Health History */
    <div key={2} className="flex flex-col gap-4">
      <TextArea label='Current Medical Conditions (or "None")' id="conditions" rows={3} value={data.conditions} onChange={handleChange("conditions")} />
      <TextArea label="Current Medications & Dosages" id="medications" rows={3} value={data.medications} onChange={handleChange("medications")} />
      <TextArea label="Past Surgeries or Procedures" id="surgeries" rows={2} value={data.surgeries} onChange={handleChange("surgeries")} />
      <Field label="Known Allergies" id="allergies" value={data.allergies} onChange={handleChange("allergies")} />
    </div>,
    ...
    <div key={3} className="flex flex-col gap-5">
      ...
      <div>
        ...
        <input type="range" min={1} max={10} value={data.stressLvl} onChange={handleChange("stressLvl")} style={{marginTop:"0.5rem"}} />
      </div>
      <div>
        <label htmlFor="exerciseFreq">Exercise Frequency</label>
        <select id="exerciseFreq" value={data.exerciseFreq} onChange={handleChange("exerciseFreq")}>
      ...
      <TextArea label="Diet Description" id="dietDesc" rows={2} placeholder="Keto, high-protein, no restrictions..." value={data.dietDesc} onChange={handleChange("dietDesc")} />
    </div>,
    ...
    <div key={4} className="flex flex-col gap-4">
      ...
      {data.goals.includes("Other") && (
        <TextArea label="Describe your goal" id="goalsOther" rows={2} value={data.goalsOther} onChange={handleChange("goalsOther")} />
      )}
    </div>,

    /* 5 Screening */
    <div key={5} className="flex flex-col gap-4">
      <TextArea label="Have you used peptides before? If yes, describe." id="screenQ1" rows={2} value={data.screenQ1} onChange={handleChange("screenQ1")} />
      <TextArea label="Do you have any active cancer diagnosis or history? Please describe." id="screenQ2" rows={2} value={data.screenQ2} onChange={handleChange("screenQ2")} />
    </div>,
    ...
    <div key={6} className="flex flex-col gap-4">
      {([
        ...
        <label key={k} style={{display:"flex",gap:"0.75rem",alignItems:"flex-start",cursor:"pointer"}}>
          <input type="checkbox" id={k} checked={data[k] as boolean} onChange={handleChange(k)}
