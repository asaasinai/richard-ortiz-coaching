...
export default function AdminSMSPage() {
  ...
  return (
    <div style={{ maxWidth:900 }}>
      <h1 style={{ fontFamily:"Inter Tight,sans-serif",fontWeight:900,fontSize:"1.5rem",marginBottom:"0.25rem" }}>SMS Builder</h1>
      <p style={{ color:"var(--text-mute)",fontSize:"0.875rem",marginBottom:"2rem" }}>PHI-free templates only. Copy to clipboard — send manually via your phone.</p>

      <div style={{ display:"grid",gridTemplateColumns:"260px 1fr",gap:"1.5rem" }}>
        {/* Template selector */}
        <div>
          <p style={{ fontSize:"0.75rem",color:"var(--gold)",fontWeight:700,textTransform:"uppercase",letterSpacing:"0.08em",marginBottom:"0.75rem" }}>Templates</p>
          <div style={{ display:"flex",flexDirection:"column",gap:"0.35rem" }}>
            {TEMPLATES.map((t,i) => (
              <button key={t.name} onClick={() => setSelected(i)} style={{
                textAlign:"left",padding:"0.6rem 0.75rem",borderRadius:"var(--radius)",cursor:"pointer",
      ...
      {log.length > 0 && (
        <div className="card" style={{ marginTop:"2rem" }}>
          ...
          <div style={{ display:"flex",flexDirection:"column",gap:"0.4rem" }}>
            {log.map((entry,i) => (
              <div key={i} style={{ display:"flex",gap:"1rem",alignItems:"flex-start",fontSize:"0.8rem",borderBottom:"1px solid var(--border)",paddingBottom:"0.4rem" }}>
                <span style={{ color:"var(--text-mute)",flexShrink:0,width:70 }}>{entry.time}</span>
                <span style={{ color:"var(--gold)",flexShrink:0,width:180 }}>{entry.template}</span>
