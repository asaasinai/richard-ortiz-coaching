    ...
    <div style={{ display:"flex",gap:"1.5rem",height:"calc(100vh - 4rem)" }}>
      <div style={{ flex:1,overflow:"auto" }}>
        <div style={{ display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:"1.5rem" }}>
          <h1 style={{ fontFamily:"Inter Tight,sans-serif",fontWeight:900,fontSize:"1.5rem" }}>Check-Ins</h1>
          <div style={{ display:"flex",gap:"0.5rem" }}>
            {(["all","urgent"] as const).map(f => (
              <button key={f} onClick={() => setFilter(f)} style={{
                padding:"0.35rem 0.9rem",borderRadius:"var(--radius)",fontSize:"0.8rem",fontWeight:600,cursor:"pointer",
            ...
            {filtered.map(c => (
              <div key={c.id} onClick={() => setSelected(c)}
                ...
                <div style={{ display:"flex",alignItems:"center",gap:"0.6rem" }}>
                  {c.urgent_flag && <AlertTriangle size={14} style={{ color:"#f87171" }} />}
                  <div>
                    <div style={{ fontWeight:600,fontSize:"0.85rem" }}>
                      Progress: {c.data.progressScore}/10 · Energy: {c.data.energyScore}/10 · Mood: {c.data.moodScore}/10
                    </div>
                    <div style={{ color:"var(--text-mute)",fontSize:"0.78rem",marginTop:"0.2rem" }}>
